...
/** Code By Ccocot | ReCode by YOGI DWI ANDRIAN **/
	...
	try {
		...
		for (let i = 0; i < getMedia.length; i++) {
			...
			await Promise.all(getMedia[i].map(async(media) => {
				...
				const PrintOut = chalk`> ${media.link} => ${doDelete ? chalk`{bold.green Succsess}` : chalk`{bold.red failed}`}`
...
{green Happy Tutorial - Yogi Dwi Andrian}
