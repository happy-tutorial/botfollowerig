...
const User = [
  ...
  },
  {
    type:'input',
    name:'target',
    message:'Target IG (without @)'
  },
  {
    type:'input',
    name:'text',
    message:'Write Comment 1 (Use [|] when more 1)'
  },
  {
    type:'input',
    name:'sleep',
...
const CommentAndLike = async function(session, accountId, text){
  ...
  try {
    ...
    if (result.length > 0) {
      ...
      return chalk`{bold.green SUCCESS [Follow,Comment,Like]} | ${text}`;
    }
    return chalk`{bold.green SUCCESS [FOLLOW]}`
  } catch (err) {
    // console.log(err);
      ...
      for (let i = 0; i < TargetResult.length; i++) {
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            console.log(chalk`{bold.yellow [SKIPPED]}${akun.params.username} => PRIVATE ACCOUNT FOLLOWED`)
          }
        }));
...
{HAPPY TUTORIAL - YOGI DWI ANDRIAN}
{bold.red Code By Ccocot | ReCode by YOGI DWI ANDRIAN}
`);
